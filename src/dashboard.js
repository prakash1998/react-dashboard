...
const getLayoutForBreakPoint = (
    ...
    return layout.map(widget => {
        if (
            layouts[forBreakpoint] &&
            layouts[forBreakpoint].filter(w => w.id === widget.id).length > 0
        ) {
            return widget
        }
        // console.log(widget)
        const { x, w } = widget
        const ratio =
            BREAKPOINTRATIOS[currentBreakpoint] / BREAKPOINTRATIOS[forBreakpoint]
        // console.log(ratio)
        // console.log('here')
        return {
            ...widget,
...
const Dashboard = props => {
    ...
    const onLayoutChange = useCallback(
        (layout, layouts) => {
            ...
            if (layout && currentBreakPoint && layouts) {
                setLayoutsState(
                    getLayoutsFromLayout(layout, layouts, currentBreakPoint)
                )
            }
        },
    ...
    return (
        <ResponsiveReactGridLayout
          ...
          compactType={
                enableGravity ? (leftGravity ? 'horizontal' : 'vertical') : null
            }
                ...
                widgets.map(widget => {
                    ...
                    if (id && Component) {
                        ...
                        if (id) {
                            ...
                            throw Error(
                                'you missed "id" for one of the Widget, so it can\'t be rendered '
                            )
                        }
                    }
                })}
