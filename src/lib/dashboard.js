...
const Dashboard = (props) => {
    ...
    // console.log( 'created ', createLayoutFromWidgets(widgets));
    ...
    useEffect(() => {
        ...
        if (savedLayout.length > 0)
            setOnScreenWidgetIds(savedLayout.map(item => item.i))
        else
    ...
    return (<div style={{ height:height + 50, backgroundColor: "red" }} >
                    ...
                    widgets && widgets.filter(widget => onScreenWidgetIds.includes(widget.id))
                        .map(widget => {
                            ...
                            >
                                ...
                                onClick={() => setOnScreenWidgetIds(ids => ids.filter(wi => wi!==id))}>x</span>}
